exports.handler = async function (event) {
  ...
  const backgroundUrl = `${siteUrl}/.netlify/functions/rescore-upcoming-background`;

  const page = (body) => ({
    statusCode: 200,
    headers: { 'Content-Type': 'text/html' },
    body: `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Rescore trigger</title>
<style>
...
<p>A batch of the upcoming rescore is now running in the background. This will take a few minutes, and nothing will appear here while it works, that's expected.</p>
<p>Check progress at the <a href="/.netlify/functions/rescore-upcoming?max=0">rescore page</a> in a few minutes. Come back here and tap again whenever you want to run another batch.</p>
