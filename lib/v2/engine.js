...
async function processRss(state, today) {
  ...
  for (const item of fresh) {
    ...
    try { x = await extractCandidate(item); }
      ...
      // Do NOT mark this link seen. A transient extraction/API failure should
      // get another chance next run rather than silently deleting the release.
      addEvent(state, 'error', `RSS extraction failed: ${item.title}`, err.message);
      continue;
    }

    if (!x.isAlbumAnnouncement || !x.artist || !x.albumTitle) {
      seen.add(item.link); // definitive non-candidate, safe to retire
      continue;
    }
    ...
    try {
      ...
      seen.add(item.link); // retire only after the release is safely in V2 state
      added++;
    } catch (err) {
      // Keep unseen so a temporary scoring/art failure can recover next run.
...
async function runEngine() {
  ...
  const summary = { rssAdded: 0, wikiAdded: 0, transitioned: 0, spotifyRetried: 0, errors: [] };

  const stage = async (name, fn) => {
    try {
      summary[name] = await fn();
      // Checkpoint after every mutating stage. A later timeout or external
      // outage cannot erase releases already discovered earlier in the run.
      await saveState(state);
    } catch (err) {
      summary.errors.push({ stage: name, error: err.message });
      addEvent(state, 'error', `Engine stage failed: ${name}.`, err.message);
      await saveState(state).catch(() => {});
    }
  };

  await stage('rssAdded', () => processRss(state, today));
  await stage('wikiAdded', () => processWikipedia(state, today));
  await stage('transitioned', () => transitionReleases(state, today));
  await stage('spotifyRetried', () => retrySpotify(state));
...
module.exports = { runEngine, todayMountain, lifecycleFor, spotifyBackoffHours };
