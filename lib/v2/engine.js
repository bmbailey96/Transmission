...
const cleanName = s => String(s || '').toLowerCase().replace(/&/g, 'and').replace(/[^a-z0-9]+/g, ' ').trim();
const confirmedPicks = new Set(picks.map(x => `${cleanName(x.artist)}|${cleanName(x.title)}`));
function spotifyEligible(r) {
  return r.lifecycle === 'released' && typeof r.score === 'number' &&
    r.score >= SCORE_FOR_SPOTIFY &&
    !confirmedPicks.has(`${cleanName(r.artist)}|${cleanName(r.albumTitle)}`);
}
...
async function scoreNew({ artist, albumTitle, releaseDate, evidenceText, link, sourceFeed, announcedAt }, today) {
  ...
  return {
    ...
    spotify: released && spotifyEligible({ artist, albumTitle, lifecycle: 'released', score: scored.score })
...
async function syncSpotify(release, state) {
  ...
  try {
    ...
    if (result.queueFull) {
      release.spotify = { status: 'pending', attempts: attempts - 1,
        nextRetryAt: new Date(Date.now() + 24 * 3600000).toISOString(),
        note: `Queue full (${result.activeTracks} tracks).` };
      return;
    }
    if (!result.found) {
    ...
    const retryMs = err?.retryAfterSeconds
      ? (err.retryAfterSeconds + 15) * 1000
      : spotifyBackoffHours(attempts) * 3600000;
    const next = new Date(Date.now() + retryMs).toISOString();
    release.spotify = {
...
async function transitionReleases(state, today) {
  ...
  for (const r of due) {
    try {
      ...
      r.spotify = spotifyEligible(r) ? { status: 'pending', attempts: 0 } : { status: 'not_needed', attempts: 0 };
      if (!r.art?.url) {
...
async function runEngine() {
  ...
  const creditFailures = (state.events || []).filter(e =>
    Date.parse(e.at) >= started && /credit balance is too low/i.test(e.detail || '')
  );
  if (creditFailures.length) summary.errors.push({ phase: 'scoring', error: 'Anthropic credits exhausted' });
...
module.exports = {
  runEngine, todayMountain, lifecycleFor, spotifyBackoffHours, safeIsoDate, SCORE_FOR_SPOTIFY, spotifyEligible, syncSpotify,
};
