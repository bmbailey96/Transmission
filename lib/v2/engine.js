...
async function seedLegacy(state, today) {
  ...
  // Do not mark migration complete if the legacy store is temporarily
  // unavailable. A failed seed must be retried on the next engine run.
  const old = await getLegacyReleases();
...
async function runEngine() {
  ...
  const summary = {
    seeded: 0, rssAdded: 0, wikiAdded: 0, transitioned: 0, spotifyRetried: 0,
    errors: [],
  };
  ...
  await runPhase(state, summary, 'legacy seed', 'seeded', () => seedLegacy(state, today));
...
module.exports = {
  runEngine, todayMountain, lifecycleFor, spotifyBackoffHours, safeIsoDate,
};
