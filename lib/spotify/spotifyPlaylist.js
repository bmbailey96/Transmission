...
const PLAYLIST_DESCRIPTION =
  'Auto-built sampler: a handful of tracks from each already-released album the radar scored well. Not a ranking, just enough to judge for yourself.';
...
async function searchAlbum(token, artist, albumTitle) {
  const q = `artist:${artist} album:${albumTitle}`;
  const data = await spotifyFetch(token, `/search?q=${encodeURIComponent(q)}&type=album&limit=5`);
...
async function addAlbumToPlaylist({ artist, albumTitle, standoutTrackNames = [] }) {
  ...
  const picked = pickTracks(tracks, standoutTrackNames, 5);
