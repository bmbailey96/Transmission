...
const PLAYLIST_DESCRIPTION =
  'A small discovery queue: one track per strong unfamiliar album. Predictions, not favorites.';
const MAX_ACTIVE_TRACKS = 50;
...
async function spotifyFetch(token, path, options = {}) {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    ...
    if (res.status === 429) {
      ...
      // Spotify's Retry-After is authoritative. The old helper capped this at
      // 12 seconds, which guaranteed another 429 whenever Spotify asked for a
      // longer cooldown. Background functions have enough room to honor a
      // reasonable cooldown directly.
      if (attempt < maxAttempts && retryAfter <= 300) {
      ...
      const err = new Error(
        `Spotify ${options.method || 'GET'} ${path} rate limited: ${res.status} ${body}`
      );
      err.status = 429;
...
function pickTracks(tracks, standoutNames = [], count = 5) {
  ...
  const pool = tracks.filter((t) => !matchedIds.has(t.id) && (t.duration_ms || 0) >= 90000);
  if (count === 1 && remainingCount && pool.length) {
...
async function addAlbumToPlaylist({ artist, albumTitle, standoutTrackNames = [] }) {
  ...
  if (alreadyInPlaylist.length >= MAX_ACTIVE_TRACKS) {
    return { found: true, queueFull: true, artist, albumTitle, activeTracks: alreadyInPlaylist.length };
  }
  ...
  const picked = pickTracks(tracks, standoutTrackNames, 1);
