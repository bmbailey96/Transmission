  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    ...
    if (res.status === 429) {
      ...
      if (attempt < maxAttempts && retryAfter <= 300) {
        // Spotify's Retry-After is authoritative. The old 12-second cap was
        // the reason every retry immediately hit the same quota window again.
        await sleep(retryAfter * 1000 + 250);
        continue;
      }
      throw new Error(
        `Spotify ${options.method || 'GET'} ${path} failed: 429 QUOTA_EXCEEDED (retry-after=${retryAfter}s)`
      );
    }
...
let cachedPlaylistUriSet = null;
let cachedPlaylistUriSetId = null;
...
async function getCurrentPlaylistUris(token, playlistId) {
  if (cachedPlaylistUriSet && cachedPlaylistUriSetId === playlistId) {
    return [...cachedPlaylistUriSet];
  }
  ...
  cachedPlaylistUriSetId = playlistId;
  cachedPlaylistUriSet = new Set(uris);
  return uris;
}
...
async function addUrisToPlaylist(token, playlistId, uris) {
  ...
  if (cachedPlaylistUriSet && cachedPlaylistUriSetId === playlistId) {
    for (const uri of uris) cachedPlaylistUriSet.add(uri);
  }
}
...
async function removeUrisFromPlaylist(token, playlistId, uris) {
  ...
  if (cachedPlaylistUriSet && cachedPlaylistUriSetId === playlistId) {
    for (const uri of uris) cachedPlaylistUriSet.delete(uri);
  }
}
