...
async function extractCandidate(item) {
  ...
  // Belt and suspenders on top of the system prompt's instruction: a
  // month-only or otherwise partial date is worse than no date at all here,
  // see isValidReleaseDate's comment for why. Null it out rather than trust
  // the model followed the YYYY-MM-DD-or-null instruction.
  if (parsed.releaseDate && !isValidReleaseDate(parsed.releaseDate)) {
