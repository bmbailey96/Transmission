...
const DEFAULT_MAX_PER_RUN = 1; // successful checks to actually land per run, see MAX_ATTEMPTS_PER_RUN
const MAX_ATTEMPTS_PER_RUN = 4; // total candidates to try per run, success or fail, same reasoning as the daily mover: a failing candidate shouldn't be able to block everything behind it
...
async function runRecheckAlreadyReleasedScores({ maxPerRun = DEFAULT_MAX_PER_RUN, maxAttempts = MAX_ATTEMPTS_PER_RUN } = {}) {
...
module.exports = { runRecheckAlreadyReleasedScores, findUncheckedHighScorers, DEFAULT_MAX_PER_RUN, MAX_ATTEMPTS_PER_RUN };
