...
const { upsertRelease, getRelease, getAllReleases, releaseKey } = require('../storage/releaseStore');
...
async function runStructuredPass(entries, maxNew = 15) {
   ...
   * fixes that specifically, a plain field merge via upsertRelease rather
   * than a rescore, since it already has a score and there's no reason to
   * spend another Anthropic call redoing that.
   */
  ...
  const backfilled = await Promise.all(
    backfillTargets.map(async (entry) => {
      try {
        await upsertRelease({ artist: entry.artist, albumTitle: entry.albumTitle, releaseDate: entry.releaseDate });
        return `${entry.artist} - ${entry.albumTitle} (releaseDate was missing, set to ${entry.releaseDate})`;
      } catch (err) {
        return `${entry.artist} - ${entry.albumTitle} (backfill failed: ${err.message})`;
      }
    })
  );
