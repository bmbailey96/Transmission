...
A second failure mode, easy to slide into once you're avoiding the first one: don't replace it with a different template, closing every "none"-evidence entry on some version of "the score rests entirely on that, which is appealing but not guaranteed" or "strong but not a sure thing." The evidenceLevel field already communicates that mechanically, you don't need a formulaic hedge restating it in prose every time. End the reasoning wherever the actual thought ends. Some should end on the comparison itself, some on a caveat, some on what would change your mind, vary it the same way you'd vary which detail you lead with.
